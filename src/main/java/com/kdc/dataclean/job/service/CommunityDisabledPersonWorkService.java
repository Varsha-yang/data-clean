...
@Service
public class CommunityDisabledPersonWorkService {
    @Autowired
    ...
    // 临时表名
    private static final String tempTableName = "base_dnt_disable";
    // 目标表
    private static final String destTableName = "new_grid_disabled_person";
    // job Id
    private static final String jobId = "CommunityDisabledPerson";
        ...
        if (srcMapList != null && srcMapList.size() > 0) {
            ...
            srcMapList.forEach(
                    recode -> {
                        ...
                        String disableTyp = CommunityDataConvertMap.disableTyp(MapUtils.getString(recode, "disable_typ")); // 残疾类型
                        String disableDt = MapUtils.getString(recode, "disable_tm"); // 残疾时间
                        if (disableDt != null && disableDt.equals("")) {
                            disableDt = null;
                        }
                        String supprt = MapUtils.getString(recode, "supprt"); // 补充说明
                        String disableLvl = CommunityDataConvertMap.disableLvl(MapUtils.getString(recode, "disable_lvl")); // 残疾等级
                        String disableNum = MapUtils.getString(recode, "disable_num"); // 残疾号
                        ...
                        if (idenNum == null || idenNum.equals("")) {
                            ...
                            if (!status.equals("F")) {
                                if (idenList.contains(idenNum)) {
                                    ...
                                    destUpdateMap.put("disabled_catg_cd", disableTyp);
                                    destUpdateMap.put("disabled_dt", disableDt);
                                    destUpdateMap.put("disabled_rsn", supprt);
                                    destUpdateMap.put("disabled_lvl_cd", disableLvl);
                                    destUpdateMap.put("disabled_cert_num", disableNum);
                                    ...
                                    destInsertMap.put("disabled_catg_cd", disableTyp);
                                    destInsertMap.put("disabled_dt", disableDt);
                                    destInsertMap.put("disabled_rsn", supprt);
                                    destInsertMap.put("disabled_lvl_cd", disableLvl);
                                    destInsertMap.put("disabled_cert_num", disableNum);
            ...
            // 数据插入到目标表
            List<String> destFieldList = Arrays.asList("iden_crd_num", "nam", "disabled_catg_cd", "disabled_dt", "disabled_rsn", "disabled_lvl_cd", "disabled_cert_num", "created_time", "created_by", "updated_by", "updated_time", "version");
